  ...
  return (
    <div ref={dropdownRef} className={`tw-relative ${width}`}>
      ...
      <button
        ...
        className="tw-h-[40px] tw-w-full tw-flex tw-items-center tw-justify-between tw-px-3 tw-py-2 tw-rounded-md tw-shadow-[0_4px_4px_0_rgba(194,194,194,0.25)] tw-text-[14px] tw-border tw-border-white tw-bg-[#f6f6f6] hover:tw-border-[#156082] tw-transition-all tw-duration-200 tw-ease-in-out"
      >
        <div className="tw-flex tw-items-center tw-gap-1 tw-flex-1 tw-min-w-0 tw-overflow-x-auto tw-scrollbar-thin tw-scrollbar-thumb-[#ccc] tw-scrollbar-track-transparent">
          <span className="tw-text-[#999] tw-truncate tw-shrink-0 tw-mr-1">{placeholder}</span>
          {selected.map((option) => (
            <span
              key={option}
      ...
      {isOpen && (
        <ul className="tw-absolute tw-left-0 tw-right-0 tw-mt-2 tw-bg-white tw-border tw-border-[#ededed] tw-rounded-md tw-shadow-[0_4px_4px_0_rgba(143,143,143,0.65)] tw-max-h-56 tw-overflow-y-auto tw-z-10 tw-animate-fadeIn">
          ...
          >
            <span className="tw-font-medium tw-text-[#156082] tw-flex-1">Select All</span>
            <input
              type="checkbox"
