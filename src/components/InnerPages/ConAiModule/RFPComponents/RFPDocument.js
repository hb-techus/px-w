...
import {
  Upload, Search, FileText, FileType, File,
  CheckCircle2, XCircle, Loader2,
  CloudUpload, X, ChevronDown, Check,
  AlertCircle, Clock3
} from "lucide-react";
import {
  GetProjectDocuments,
...
import RfpHighlightViewer from "./RfpHighlightViewer";
...
function StatusBadge({ code }) {
  ...
  return (
    <span className={`tw-inline-flex tw-items-center tw-gap-1.5 tw-px-3 tw-py-1 tw-rounded-full tw-text-[13px] tw-font-medium tw-border ${bg} ${border} ${text}`}>
      {typeof Icon === "string" ? (
        <i className={Icon} style={{ fontSize: 14, lineHeight: 1 }} />
      ) : Icon ? (
...
const SCROLLBAR_STYLE = `
  .rfp-scroll::-webkit-scrollbar { width: 8px; height: 8px; }
  .rfp-scroll::-webkit-scrollbar-track { background: transparent; }
  .rfp-scroll::-webkit-scrollbar-thumb { background-color: #dee9ff; border-radius: 99px; }
  .rfp-scroll { scrollbar-color: #dee9ff transparent; scrollbar-width: thin; }
`;
...
const getFileIcon = (type) => {
  ...
  if (ext === "txt") return <File className="tw-w-4 tw-h-4 tw-text-gray-500" />;
  return <File className="tw-w-4 tw-h-4" />;
};
...
export default function Documents() {
  ...
  const { permissions, packagePermissions } = usePermissions('rfp_file_manager', 'bid_intelligence');
  const [viewingDoc, setViewingDoc] = useState(null);
  const packageList = useSelector((s) => s?.auth?.user?.[0]?.package_info);
const { isMarkAsCompleted} = useEstimation();
console.log(isMarkAsCompleted);
  const rfpUploadLimit = useMemo(() => {
  ...
  const uploadButtonDisabled = isUploading || !allowUpload;
...
const handleDrop = useCallback((e) => {
  ...
  if (!dropped.length) { showToast("error", "Only PDF files are allowed"); return; }
  setSelectedFiles(prev => {
    const effectiveMax = rfpUploadLimit ?? MAX_FILES;
const alreadyUploaded = documents.length; // ← existing docs in table
const rem = effectiveMax - prev.length - alreadyUploaded;
    if (rem <= 0) {
      setUpgradeMessage(`Your current package allows up to ${effectiveMax} RFP file${effectiveMax > 1 ? "s" : ""}. Upgrade your package to upload more.`);
      setShowUpgradeModal(true);
      return prev;
    }
    if (dropped.length > rem) {
      setUpgradeMessage(`Your current package allows up to ${effectiveMax} RFP file${effectiveMax > 1 ? "s" : ""}. Upgrade your package to upload more.`);
      setShowUpgradeModal(true);
    }
    return [...prev, ...dropped.slice(0, rem)];
  });
}, [rfpUploadLimit, setUpgradeMessage, setShowUpgradeModal]);
...
const rem = effectiveMax - prev.length - alreadyUploaded;
    if (rem <= 0) {
// AFTER — better message
setUpgradeMessage(
  `Your current package allows up to ${effectiveMax} RFP file${effectiveMax > 1 ? "s" : ""}. You already have ${alreadyUploaded} uploaded. Upgrade your package to upload more.`
);      setShowUpgradeModal(true);
      return prev;
    }
...
setUpgradeMessage(
  ...
  const uploadSingleFile = useCallback(async (file) => {
    ...
    try {
      ...
      const urlRes = await GetUploadUrl({ organization_uuid, project_uuid, file_name: file.name, module_type: "RFP" });
      ...
      const confirmRes = await ConfirmUpload({
        ...
        original_file_name: file.name, document_uuid, module_type: "RFP",
  ...
  const handleDelete = useCallback(async (doc) => {
    const organization_uuid = localStorage.getItem("organization_uuid");
    const project_uuid = localStorage.getItem("project_uuid");
    setDeletingIds(p => ({ ...p, [doc.document_id]: true }));
    setPageLoading(true);
    try {
      await DeleteProjectDocument({
        organization_uuid, project_uuid,
        document_uuid: doc.document_id, device_info: getDeviceInfo(),
      });
      showToast("success", "Document deleted");
      await fetchDocuments();
    } catch (err) { showToast("error", err.message); }
    finally {
      setDeletingIds(p => { const n = { ...p }; delete n[doc.document_id]; return n; });
      setPageLoading(false);
    }
  }, [fetchDocuments]);
  ...
  const columns = useMemo(() => [
    {
      ...
      cell: (row) => (
        <div className="tw-flex tw-items-center tw-gap-3">
          <div className="tw-w-8 tw-h-8 tw-rounded-lg tw-flex tw-items-center tw-justify-center tw-flex-shrink-0">
            {getFileIcon((row.document_name || "").split(".").pop().toLowerCase())}
          </div>
          <p className="tw-font-bold tw-text-[15px] tw-text-[#585858] tw-truncate tw-max-w-[180px]">
            {row.document_name}
          </p>
        </div>
      ),
        ...
        return (
          <ActionMenu
            ...
            onDelete={() => handleDelete(row)}
             deleteDisabled={isMarkAsCompleted || true}
          />
        );
      },
    },
  ...
  function PaginationBar() {
    ...
    return (
      <div className="tw-flex tw-items-center tw-justify-between tw-py-4 !tw-mt-0">
        <p className="tw-text-sm tw-text-[#999]"></p>
  ...
  return (
    <div className="tw-p-1 tw-flex-1">
      ...
      {isUploadModalOpen && (
        <UploadModalContent
          {...{ isUploading, selectedFiles, setSelectedFiles, uploadProgressMap, isDragging, handleDragOver, handleDragLeave, handleDrop, fileInputRef, handleFileSelect, removeFile, handleUpload }}
        ...
        <div className="tw-flex tw-items-center tw-justify-between">
          <div>
            <div className="tw-flex tw-items-center tw-gap-2">
              ...
              <span className="tw-text-[20px] tw-font-bold tw-text-gray-900">RFP File Manager</span>
            </div>
            <p className="tw-text-[#1e293b] tw-text-[14px]">Upload, manage, and track the processing status of all RFP documents tied to your project.</p>
          </div>
          {permissions?.upload && (
  <UploadButton disabled={uploadButtonDisabled || isMarkAsCompleted} 
        ...
        <div className="tw-bg-white tw-border tw-border-[#e0e0e0] tw-rounded-[15px] tw-overflow-hidden">
          ...
          <div className="project-table">
            <CustomDataTable
              ...
              noDataComponent={
                <NoDataFound
                  description={hasActiveFilters ? "No documents match your filters." : "No documents found. Upload your first RFP document to get started."}
...
function UploadButton({ disabled, onClick, isMarkAsCompleted }) {
  return (
    <button
      ...
      className={`tw-px-4 tw-py-2 tw-text-sm tw-font-medium tw-text-white tw-rounded-md tw-transition-opacity tw-flex tw-items-center tw-gap-2
    ...
    <div className="tw-fixed tw-inset-0 tw-bg-black tw-bg-opacity-50 tw-z-50 tw-flex tw-items-center tw-justify-center">
      <div className="tw-bg-white tw-rounded-lg tw-max-w-lg tw-w-full tw-mx-4 tw-shadow-lg">
        <div className="tw-border-b tw-px-6 tw-py-4">
          <div className="tw-flex tw-items-center tw-gap-2">
            {isUploading ? <Loader2 className="tw-w-5 tw-h-5 tw-text-blue-600 tw-animate-spin" /> : <Upload className="tw-w-5 tw-h-5 tw-text-blue-600" />}
            <h2 className="tw-text-lg tw-font-semibold">{isUploading ? "Uploading..." : "Upload Documents"}</h2>
          </div>
          <p className="tw-text-sm tw-text-gray-600 tw-mt-1">
            {isUploading ? "Please wait while your files are being uploaded" : "Drag and drop files or browse to upload your RFP documents"}
          </p>
        </div>
        ...
        <div className="tw-p-6 tw-space-y-4">
          ...
          >
            <input type="file" ref={fileInputRef} onChange={handleFileSelect} multiple accept=".pdf" className="tw-hidden" />
            <CloudUpload className={`tw-w-12 tw-h-12 tw-mx-auto tw-mb-4 ${isDragging ? "tw-text-blue-600" : "tw-text-gray-400"}`} />
            <p className="tw-text-sm tw-font-medium tw-text-gray-700 tw-mb-1">{isDragging ? "Drop files here" : "Drag & drop files here"}</p>
            <p className="tw-text-xs tw-text-gray-500 tw-mb-3">or</p>
            <button
              type="button" disabled={isUploading || selectedFiles.length >= MAX_FILES}
            ...
            >Browse Files</button>
            {/* <p className="tw-text-xs tw-text-gray-500 tw-mt-3">PDF only · Max {MAX_FILES} files · 110MB each</p> */}
          </div>

          {selectedFiles.length > 0 && (
            <div className="tw-space-y-2 tw-max-h-40 tw-overflow-y-auto">
              {/* <p className="tw-text-sm tw-font-medium tw-text-gray-700">Selected Files ({selectedFiles.length}/{MAX_FILES})</p> */}
                 <p className="tw-text-sm tw-font-medium tw-text-gray-700">Selected Files </p>
                ...
                return (
                  <div key={index} className="tw-flex tw-items-center tw-justify-between tw-p-2 tw-bg-gray-100 tw-rounded-md">
                    ...
                    <div className="tw-flex tw-items-center tw-gap-2 tw-ml-2 tw-flex-shrink-0">
                      {prog ? (
                        prog.error ? <span className="tw-text-xs tw-text-red-500">{prog.error}</span>
                          : prog.done ? <CheckCircle2 className="tw-w-4 tw-h-4 tw-text-green-500" />
                            : <span className="tw-text-xs tw-text-blue-600">{prog.progress}%</span>
                      ) : (
                        <button disabled={isUploading} onClick={() => removeFile(index)}
        ...
        <div className="tw-flex tw-justify-end tw-gap-2 tw-p-6 tw-border-t">
          <button disabled={isUploading} onClick={onClose}
            className="tw-px-4 tw-py-2 tw-text-sm tw-font-medium tw-border tw-bg-[#dedede] tw-border-gray-300 tw-rounded-[5px] hover:tw-bg-gray-50 tw-transition-colors disabled:tw-opacity-50">
            Cancel
          </button>
          <button onClick={handleUpload} disabled={!selectedFiles.length || isUploading}
            className="tw-px-4 tw-py-2 tw-text-sm tw-font-medium tw-text-white tw-bg-blue-600 tw-rounded-[5px] hover:tw-bg-blue-700 disabled:tw-opacity-50 disabled:tw-cursor-not-allowed tw-transition-colors tw-flex tw-items-center tw-gap-2">
